...
import { getCurrDayAndMonth } from "./common.html";
    ...
    if (config.disableEventLink || event.htmlLink == 'undefined' || event.htmlLink === null)
        return html`
				<div style="text-decoration: ${textDecoration};color: ${titleColor}">
					<div class="${dayClassEventRunning}" style="--event-title-size: ${config.eventTitleSize}%">${getEventIcon(config, event)} ${titletext}</div>
				</div>
			`;
    else
        return html`
				<a href="${event.htmlLink}" style="text-decoration: ${textDecoration};" target="${config.linkTarget}">
					<div style="color: ${titleColor}">
						<div class="${dayClassEventRunning}" style="--event-title-size: ${config.eventTitleSize}%">${getEventIcon(config, event)} <span>${titletext}</span></div>
					</div>
				</a>
			`;
}
...
export function getLocationHTML(config: atomicCardConfig, event: EventClass) {
    if (!event.location || !config.showLocation) {
        ...
        return html`
				<div><ha-icon class="event-location-icon" style="--location-icon-color: ${config.locationIconColor}" icon="mdi:map-marker"></ha-icon>&nbsp;${event.address}</div>
			`;
    } else {
        ...
        return html`
				<div>
					<a href=${location} target="${config.linkTarget
            } " class="location - link" style="--location - link - size: ${config.locationTextSize}% ">
